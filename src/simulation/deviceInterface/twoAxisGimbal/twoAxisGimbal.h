...
private:
    ...
    std::pair<double, double> motorAnglesToGimbalAngles(double motor1Angle,
                                                        double motor2Angle);                     //!< Method to determine the sequential gimbal tip and tilt angles given the stepper motor angles
    bool noInterpolationRequired(double motor1Angle,
                                 double motor2Angle);                                            //!< Method to determine no interpolation is required
    bool bilinearInterpolationRequired(double motor1Angle,
                                       double motor2Angle);
    bool linearInterpolationRequired(double motorAngle);                                         //!< Method to determine if linear interpolation is required
    std::pair<double, double> bilinearlyInterpolateGimbalAngles(double motor1Angle,
                                                                double motor2Angle);             //!< Method calls a bilinear interpolation function to interpolate the motor angles to gimbal angles
    std::pair<double, double> linearlyInterpolateGimbalAnglesMotor1Fixed(double motor1Angle,
                                                                         double motor2Angle);    //!< Method calls a linear interpolation function to interpolate the motor angles to gimbal angles using a fixed motor 1 angle and bounded motor 2 angle
    std::pair<double, double> linearlyInterpolateGimbalAnglesMotor2Fixed(double motor1Angle,
                                                                         double motor2Angle);    //!< Method calls a linear interpolation function to interpolate the motor angles to gimbal angles using a fixed motor 2 angle and bounded motor 1 angle
    void computeGimbalActuationParameters();                                                     //!< Method used to compute and update the gimbal actuation parameters for each segment of required gimbal motion
    void computeSingleSegmentParameters();                                                       //!< Method computes the gimbal actuation parameters for a single actuation segment (motors actuate the same number of steps)
    void computeSegment1Parameters();                                                            //!< Method computes the gimbal actuation parameters for the first of two actuation segments (motors actuate different number of steps)
    void computeSegment2Parameters();                                                            //!< Method computes the gimbal actuation parameters for the second of two actuation segments (motors actuate different number of steps)
