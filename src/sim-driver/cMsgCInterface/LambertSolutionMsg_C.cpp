...
#include "LambertSolutionMsg_C.h"
#include "architecture/messaging/messaging.h"
#include "architecture/utilities/bsk_Print.h"
#include<string.h>

//! C interface to subscribe to a message
void LambertSolutionMsg_C_subscribe(LambertSolutionMsg_C *subscriber, LambertSolutionMsg_C *source) {
...
int8_t LambertSolutionMsg_C_isSubscribedTo(LambertSolutionMsg_C *subscriber, LambertSolutionMsg_C *source) {
...
void LambertSolutionMsg_C_addAuthor(LambertSolutionMsg_C *coownerMsg, LambertSolutionMsg_C *targetMsg) {
...
void LambertSolutionMsg_C_init(LambertSolutionMsg_C *owner) {
    //! check if the msg pointer is not assigned already.  If not, then connect message to itself.
    if (owner->payloadPointer == 0) {
        LambertSolutionMsg_C_addAuthor(owner, owner);
    }
};

//! C interface to write to a message
void LambertSolutionMsg_C_write(LambertSolutionMsgPayload *data, LambertSolutionMsg_C *destination, int64_t moduleID, uint64_t callTime) {
...
LambertSolutionMsgPayload LambertSolutionMsg_C_zeroMsgPayload() {
    LambertSolutionMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(LambertSolutionMsgPayload));
    return zeroMsg;
};


//! C interface to read to a message
LambertSolutionMsgPayload LambertSolutionMsg_C_read(LambertSolutionMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type LambertSolutionMsg.");
    }
...
int LambertSolutionMsg_C_isLinked(LambertSolutionMsg_C *data) {
    return (int) data->header.isLinked;
};

//! C interface to see if this message container ever been written to
int LambertSolutionMsg_C_isWritten(LambertSolutionMsg_C *data) {
    if (data->header.isLinked) {
        return (int) data->headerPointer->isWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type LambertSolutionMsg is written.");
    return 0;
};

//! C interface to see if this message container ever been written to
uint64_t LambertSolutionMsg_C_timeWritten(LambertSolutionMsg_C *data) {
    if (data->header.isLinked) {
        return data->headerPointer->timeWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type LambertSolutionMsg.");
    return 0;
};

//! C interface to get the moduleID of who wrote the message
int64_t LambertSolutionMsg_C_moduleID(LambertSolutionMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type LambertSolutionMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type LambertSolutionMsg.");
        return 0;
    }
...
//! method description
void LambertSolutionMsg_cpp_subscribe(LambertSolutionMsg_C *subscriber, void* source){
    Message<LambertSolutionMsgPayload>* source_t = (Message<LambertSolutionMsgPayload>*) source;
    MsgHeader *msgPtr;
...
int8_t LambertSolutionMsg_cpp_isSubscribedTo(LambertSolutionMsg_C *subscriber, void* source) {
