...
#include "LambertValidatorMsg_C.h"
#include "architecture/messaging/messaging.h"
#include "architecture/utilities/bsk_Print.h"
#include<string.h>

//! C interface to subscribe to a message
void LambertValidatorMsg_C_subscribe(LambertValidatorMsg_C *subscriber, LambertValidatorMsg_C *source) {
...
int8_t LambertValidatorMsg_C_isSubscribedTo(LambertValidatorMsg_C *subscriber, LambertValidatorMsg_C *source) {
...
void LambertValidatorMsg_C_addAuthor(LambertValidatorMsg_C *coownerMsg, LambertValidatorMsg_C *targetMsg) {
...
void LambertValidatorMsg_C_init(LambertValidatorMsg_C *owner) {
    //! check if the msg pointer is not assigned already.  If not, then connect message to itself.
    if (owner->payloadPointer == 0) {
        LambertValidatorMsg_C_addAuthor(owner, owner);
    }
};

//! C interface to write to a message
void LambertValidatorMsg_C_write(LambertValidatorMsgPayload *data, LambertValidatorMsg_C *destination, int64_t moduleID, uint64_t callTime) {
...
LambertValidatorMsgPayload LambertValidatorMsg_C_zeroMsgPayload() {
    LambertValidatorMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(LambertValidatorMsgPayload));
    return zeroMsg;
};


//! C interface to read to a message
LambertValidatorMsgPayload LambertValidatorMsg_C_read(LambertValidatorMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type LambertValidatorMsg.");
    }
...
int LambertValidatorMsg_C_isLinked(LambertValidatorMsg_C *data) {
    return (int) data->header.isLinked;
};

//! C interface to see if this message container ever been written to
int LambertValidatorMsg_C_isWritten(LambertValidatorMsg_C *data) {
    if (data->header.isLinked) {
        return (int) data->headerPointer->isWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type LambertValidatorMsg is written.");
    return 0;
};

//! C interface to see if this message container ever been written to
uint64_t LambertValidatorMsg_C_timeWritten(LambertValidatorMsg_C *data) {
    if (data->header.isLinked) {
        return data->headerPointer->timeWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type LambertValidatorMsg.");
    return 0;
};

//! C interface to get the moduleID of who wrote the message
int64_t LambertValidatorMsg_C_moduleID(LambertValidatorMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type LambertValidatorMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type LambertValidatorMsg.");
        return 0;
    }
...
//! method description
void LambertValidatorMsg_cpp_subscribe(LambertValidatorMsg_C *subscriber, void* source){
    Message<LambertValidatorMsgPayload>* source_t = (Message<LambertValidatorMsgPayload>*) source;
    MsgHeader *msgPtr;
...
int8_t LambertValidatorMsg_cpp_isSubscribedTo(LambertValidatorMsg_C *subscriber, void* source) {
