...
#include "CameraLocalizationMsg_C.h"
...
void CameraLocalizationMsg_C_addAuthor(CameraLocalizationMsg_C *coownerMsg, CameraLocalizationMsg_C *targetMsg) {
...
void CameraLocalizationMsg_C_init(CameraLocalizationMsg_C *owner) {
    ...
    if (owner->payloadPointer == 0) {
        CameraLocalizationMsg_C_addAuthor(owner, owner);
    }
};
...
CameraLocalizationMsgPayload CameraLocalizationMsg_C_zeroMsgPayload() {
    CameraLocalizationMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(CameraLocalizationMsgPayload));
    return zeroMsg;
};
...
CameraLocalizationMsgPayload CameraLocalizationMsg_C_read(CameraLocalizationMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type CameraLocalizationMsg.");
    }
...
int CameraLocalizationMsg_C_isWritten(CameraLocalizationMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type CameraLocalizationMsg is written.");
    return 0;
};
...
uint64_t CameraLocalizationMsg_C_timeWritten(CameraLocalizationMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type CameraLocalizationMsg.");
    return 0;
};
...
int64_t CameraLocalizationMsg_C_moduleID(CameraLocalizationMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type CameraLocalizationMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type CameraLocalizationMsg.");
        return 0;
    }
...
void CameraLocalizationMsg_cpp_subscribe(CameraLocalizationMsg_C *subscriber, void* source){
    Message<CameraLocalizationMsgPayload>* source_t = (Message<CameraLocalizationMsgPayload>*) source;
    MsgHeader *msgPtr;
