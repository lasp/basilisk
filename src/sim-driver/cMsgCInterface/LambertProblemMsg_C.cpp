...
void LambertProblemMsg_C_addAuthor(LambertProblemMsg_C *coownerMsg, LambertProblemMsg_C *targetMsg) {
...
void LambertProblemMsg_C_init(LambertProblemMsg_C *owner) {
    ...
    if (owner->payloadPointer == 0) {
        LambertProblemMsg_C_addAuthor(owner, owner);
    }
};
...
LambertProblemMsgPayload LambertProblemMsg_C_zeroMsgPayload() {
    LambertProblemMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(LambertProblemMsgPayload));
    return zeroMsg;
};
...
LambertProblemMsgPayload LambertProblemMsg_C_read(LambertProblemMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type LambertProblemMsg.");
    }
...
int LambertProblemMsg_C_isWritten(LambertProblemMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type LambertProblemMsg is written.");
    return 0;
};
...
uint64_t LambertProblemMsg_C_timeWritten(LambertProblemMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type LambertProblemMsg.");
    return 0;
};
...
int64_t LambertProblemMsg_C_moduleID(LambertProblemMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type LambertProblemMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type LambertProblemMsg.");
        return 0;
    }
...
void LambertProblemMsg_cpp_subscribe(LambertProblemMsg_C *subscriber, void* source){
    Message<LambertProblemMsgPayload>* source_t = (Message<LambertProblemMsgPayload>*) source;
    MsgHeader *msgPtr;
