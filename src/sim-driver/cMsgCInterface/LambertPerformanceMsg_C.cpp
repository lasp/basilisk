...
#include "LambertPerformanceMsg_C.h"
...
void LambertPerformanceMsg_C_addAuthor(LambertPerformanceMsg_C *coownerMsg, LambertPerformanceMsg_C *targetMsg) {
...
void LambertPerformanceMsg_C_init(LambertPerformanceMsg_C *owner) {
    ...
    if (owner->payloadPointer == 0) {
        LambertPerformanceMsg_C_addAuthor(owner, owner);
    }
};
...
LambertPerformanceMsgPayload LambertPerformanceMsg_C_zeroMsgPayload() {
    LambertPerformanceMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(LambertPerformanceMsgPayload));
    return zeroMsg;
};
...
LambertPerformanceMsgPayload LambertPerformanceMsg_C_read(LambertPerformanceMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type LambertPerformanceMsg.");
    }
...
int LambertPerformanceMsg_C_isWritten(LambertPerformanceMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type LambertPerformanceMsg is written.");
    return 0;
};
...
uint64_t LambertPerformanceMsg_C_timeWritten(LambertPerformanceMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type LambertPerformanceMsg.");
    return 0;
};
...
int64_t LambertPerformanceMsg_C_moduleID(LambertPerformanceMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type LambertPerformanceMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type LambertPerformanceMsg.");
        return 0;
    }
...
void LambertPerformanceMsg_cpp_subscribe(LambertPerformanceMsg_C *subscriber, void* source){
    Message<LambertPerformanceMsgPayload>* source_t = (Message<LambertPerformanceMsgPayload>*) source;
    MsgHeader *msgPtr;
