...
#include "PrescribedTransMsg_C.h"
...
void PrescribedTransMsg_C_addAuthor(PrescribedTransMsg_C *coownerMsg, PrescribedTransMsg_C *targetMsg) {
...
void PrescribedTransMsg_C_init(PrescribedTransMsg_C *owner) {
    ...
    if (owner->payloadPointer == 0) {
        PrescribedTransMsg_C_addAuthor(owner, owner);
    }
};
...
PrescribedTransMsgPayload PrescribedTransMsg_C_zeroMsgPayload() {
    PrescribedTransMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(PrescribedTransMsgPayload));
    return zeroMsg;
};
...
PrescribedTransMsgPayload PrescribedTransMsg_C_read(PrescribedTransMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type PrescribedTransMsg.");
    }
...
int PrescribedTransMsg_C_isWritten(PrescribedTransMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type PrescribedTransMsg is written.");
    return 0;
};
...
uint64_t PrescribedTransMsg_C_timeWritten(PrescribedTransMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type PrescribedTransMsg.");
    return 0;
};
...
int64_t PrescribedTransMsg_C_moduleID(PrescribedTransMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type PrescribedTransMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type PrescribedTransMsg.");
        return 0;
    }
...
void PrescribedTransMsg_cpp_subscribe(PrescribedTransMsg_C *subscriber, void* source){
    Message<PrescribedTransMsgPayload>* source_t = (Message<PrescribedTransMsgPayload>*) source;
    MsgHeader *msgPtr;
