...
#include "DesiredVelocityMsg_C.h"
#include "architecture/messaging/messaging.h"
#include "architecture/utilities/bsk_Print.h"
#include<string.h>

//! C interface to subscribe to a message
void DesiredVelocityMsg_C_subscribe(DesiredVelocityMsg_C *subscriber, DesiredVelocityMsg_C *source) {
...
int8_t DesiredVelocityMsg_C_isSubscribedTo(DesiredVelocityMsg_C *subscriber, DesiredVelocityMsg_C *source) {
...
void DesiredVelocityMsg_C_addAuthor(DesiredVelocityMsg_C *coownerMsg, DesiredVelocityMsg_C *targetMsg) {
...
void DesiredVelocityMsg_C_init(DesiredVelocityMsg_C *owner) {
    //! check if the msg pointer is not assigned already.  If not, then connect message to itself.
    if (owner->payloadPointer == 0) {
        DesiredVelocityMsg_C_addAuthor(owner, owner);
    }
};

//! C interface to write to a message
void DesiredVelocityMsg_C_write(DesiredVelocityMsgPayload *data, DesiredVelocityMsg_C *destination, int64_t moduleID, uint64_t callTime) {
...
DesiredVelocityMsgPayload DesiredVelocityMsg_C_zeroMsgPayload() {
    DesiredVelocityMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(DesiredVelocityMsgPayload));
    return zeroMsg;
};


//! C interface to read to a message
DesiredVelocityMsgPayload DesiredVelocityMsg_C_read(DesiredVelocityMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type DesiredVelocityMsg.");
    }
...
int DesiredVelocityMsg_C_isLinked(DesiredVelocityMsg_C *data) {
    return (int) data->header.isLinked;
};

//! C interface to see if this message container ever been written to
int DesiredVelocityMsg_C_isWritten(DesiredVelocityMsg_C *data) {
    if (data->header.isLinked) {
        return (int) data->headerPointer->isWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type DesiredVelocityMsg is written.");
    return 0;
};

//! C interface to see if this message container ever been written to
uint64_t DesiredVelocityMsg_C_timeWritten(DesiredVelocityMsg_C *data) {
    if (data->header.isLinked) {
        return data->headerPointer->timeWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type DesiredVelocityMsg.");
    return 0;
};

//! C interface to get the moduleID of who wrote the message
int64_t DesiredVelocityMsg_C_moduleID(DesiredVelocityMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type DesiredVelocityMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type DesiredVelocityMsg.");
        return 0;
    }
...
//! method description
void DesiredVelocityMsg_cpp_subscribe(DesiredVelocityMsg_C *subscriber, void* source){
    Message<DesiredVelocityMsgPayload>* source_t = (Message<DesiredVelocityMsgPayload>*) source;
    MsgHeader *msgPtr;
...
int8_t DesiredVelocityMsg_cpp_isSubscribedTo(DesiredVelocityMsg_C *subscriber, void* source) {
