 ...
 */
void SpacecraftPointing::Reset(uint64_t callTime)
{
    // check if the required input messages are included
    if (!this->chiefPositionInMsg.isLinked()) {
        this->bskLogger.bskLog(BSK_ERROR, "Error: spacecraftPointing.chiefPositionInMsg wasn't connected.");
    }
    if (!this->deputyPositionInMsg.isLinked()) {
        this->bskLogger.bskLog(BSK_ERROR, "Error: spacecraftPointing.deputyPositionInMsg wasn't connected.");
    }
    ...
    v3Normalize(this->alignmentVector_B, dcm_AB[0]);
    ...
    v3Scale(-1, sigma_AB, this->sigma_BA);

    /* Set initial values of the sigma and omega of the previous timestep to zero. */
    v3SetZero(this->old_sigma_RN);
    v3SetZero(this->old_omega_RN_N);
...
void SpacecraftPointing::UpdateState(uint64_t callTime)
{
    ...
    /* read in messages */
    chiefTransMsg = this->chiefPositionInMsg();
    deputyTransMsg = this->deputyPositionInMsg();
    ...
    v3Subtract(sigma_RN, this->old_sigma_RN, delta_sigma_RN);
    MRPswitch(this->old_sigma_RN, 0.0, old_sigma_RN_shadow);
    ...
    if (v3Norm(delta_sigma_RN) >= v3Norm(delta_sigma_RN_shadow)){
        ...
        v3Copy(old_sigma_RN_shadow, this->old_sigma_RN);
        }
    ...
    old_sigma_RN_squared = this->old_sigma_RN[0]*this->old_sigma_RN[0] + this->old_sigma_RN[1]*this->old_sigma_RN[1] + this->old_sigma_RN[2]*this->old_sigma_RN[2];
    sigma_RN_squared = sigma_RN[0]*sigma_RN[0] + sigma_RN[1]*sigma_RN[1] + sigma_RN[2]*sigma_RN[2];

    m33Set(1.0 - old_sigma_RN_squared + 2.0*this->old_sigma_RN[0]*this->old_sigma_RN[0], 2.0*(this->old_sigma_RN[0]*this->old_sigma_RN[1] - this->old_sigma_RN[2]), 2.0*(this->old_sigma_RN[0]*this->old_sigma_RN[2] + this->old_sigma_RN[1]),
        2.0*(this->old_sigma_RN[1]*this->old_sigma_RN[0] + this->old_sigma_RN[2]), 1.0 - old_sigma_RN_squared + 2.0*this->old_sigma_RN[1]*this->old_sigma_RN[1], 2.0*(this->old_sigma_RN[1]*this->old_sigma_RN[2] - this->old_sigma_RN[0]),
        2.0*(this->old_sigma_RN[2]*this->old_sigma_RN[0] - this->old_sigma_RN[1]), 2.0*(this->old_sigma_RN[2]*this->old_sigma_RN[1] + this->old_sigma_RN[0]), 1.0 - old_sigma_RN_squared + 2.0*this->old_sigma_RN[2]*this->old_sigma_RN[2],
        old_B_sigma_RN);
    ...
    v3Subtract(omega_RN_N, this->old_omega_RN_N, delta_omega_RN_N);
    v3Scale((1.0/dt), delta_omega_RN_N, domega_RN_N);
    v3Copy(omega_RN_N, this->old_omega_RN_N);

    /* Copy the sigma_RN variable to the old_sigma_RN variable. */
    v3Copy(sigma_RN, this->old_sigma_RN);
    ...
    addMRP(sigma_RN, this->sigma_BA, sigma_R1N);
    v3Copy(sigma_R1N, attReferenceOutBuffer.sigma_RN);
    v3Copy(omega_RN_N, attReferenceOutBuffer.omega_RN_N);
