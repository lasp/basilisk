...
void MeanOEFeedback::Reset(uint64_t callTime) {
    ...
    if (!this->deputyTransInMsg.isLinked()) {
        this->bskLogger.bskLog(BSK_ERROR, "Error: meanOEFeedback.deputyTransInMsg wasn't connected.");
    }

    if (this->mu <= 0.0) {
        this->bskLogger.bskLog(BSK_ERROR, "Error in meanOEFeedback: mu must be set to a positive value.");
    }
    if (this->req <= 0.0) {
        this->bskLogger.bskLog(BSK_ERROR, "Error in meanOEFeedback: req must be set to a positive value.");
    }
    if (this->J2 <= 0.0) {
        this->bskLogger.bskLog(BSK_ERROR, "Error in meanOEFeedback: J2 must be set to a positive value.");
    }
...
void MeanOEFeedback::UpdateState(uint64_t callTime) {
    ...
    chiefTransMsg = this->chiefTransInMsg();
    deputyTransMsg = this->deputyTransInMsg();
...
void MeanOEFeedback::calcLyapunovFeedback(NavTransMsgPayload chiefTransMsg,
    ...
    rv2elem(this->mu, deputyTransMsg.r_BN_N, deputyTransMsg.v_BN_N, &oe_cl_osc_d);
    // osculating classic oe to mean classic oe
    ClassicElements oe_cl_mean_c, oe_cl_mean_d;
    clMeanOscMap(this->req, this->J2, &oe_cl_osc_c, &oe_cl_mean_c, -1);
    clMeanOscMap(this->req, this->J2, &oe_cl_osc_d, &oe_cl_mean_d, -1);
    ...
    if (this->oeType == 0) {
        ...
        oed[0] = (oe_cl_mean_d.a - oe_cl_mean_c.a) / oe_cl_mean_c.a - this->targetDiffOeMean[0];
        oed[1] = oe_cl_mean_d.e - oe_cl_mean_c.e - this->targetDiffOeMean[1];
        oed[2] = oe_cl_mean_d.i - oe_cl_mean_c.i - this->targetDiffOeMean[2];
        oed[3] = oe_cl_mean_d.Omega - oe_cl_mean_c.Omega - this->targetDiffOeMean[3];
        oed[4] = oe_cl_mean_d.omega - oe_cl_mean_c.omega - this->targetDiffOeMean[4];
        ...
        oed[5] = M_mean_d - M_mean_c - this->targetDiffOeMean[5];
        ...
        calc_B_cl(this->mu, oe_cl_mean_d, B);
    } else if (this->oeType == 1) {
        ...
        oed[0] = (oe_eq_mean_d.a - oe_eq_mean_c.a) / oe_eq_mean_c.a - this->targetDiffOeMean[0];
        oed[1] = oe_eq_mean_d.P1 - oe_eq_mean_c.P1 - this->targetDiffOeMean[1];
        oed[2] = oe_eq_mean_d.P2 - oe_eq_mean_c.P2 - this->targetDiffOeMean[2];
        oed[3] = oe_eq_mean_d.Q1 - oe_eq_mean_c.Q1 - this->targetDiffOeMean[3];
        oed[4] = oe_eq_mean_d.Q2 - oe_eq_mean_c.Q2 - this->targetDiffOeMean[4];
        oed[5] = oe_eq_mean_d.l - oe_eq_mean_c.l - this->targetDiffOeMean[5];
        oed[5] = adjust_range(-M_PI, M_PI, oed[5]);
        // calculate control matrix B
        calc_B_eq(this->mu, oe_eq_mean_d, B);
    }
    // calculate Lyapunov Feedback Control
    double K_oed[6];
    m66MultV6(RECAST6X6 this->K, oed, K_oed);
