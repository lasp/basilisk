...
void SpacecraftReconfig::Reset(uint64_t callTime)
{
    ...
    if (!this->deputyTransInMsg.isLinked()) {
        this->bskLogger.bskLog(BSK_ERROR, "Error: spacecraftReconfig.deputyTransInMsg wasn't connected.");
    }
    if (!this->thrustConfigInMsg.isLinked()) {
        this->bskLogger.bskLog(BSK_ERROR, "Error: spacecraftReconfig.thrustConfigInMsg wasn't connected.");
    }
    if (!this->vehicleConfigInMsg.isLinked()) {
        this->bskLogger.bskLog(BSK_ERROR, "Error: spacecraftReconfig.vehicleConfigInMsg wasn't connected.");
    }
...
void SpacecraftReconfig::UpdateState(uint64_t callTime)
{
    ...
    chiefTransMsgBuffer = this->chiefTransInMsg();
    deputyTransMsgBuffer = this->deputyTransInMsg();
    thrustConfigMsgBuffer = this->thrustConfigInMsg();
    vehicleConfigMsgBuffer = this->vehicleConfigInMsg();

    if (this->attRefInIsLinked) {
 ...
 */
void SpacecraftReconfig::UpdateManeuver(NavTransMsgPayload chiefTransMsgBuffer,
                                        NavTransMsgPayload deputyTransMsgBuffer,
                                        AttRefMsgPayload attRefInMsgBuffer,
                                        THRArrayConfigMsgPayload thrustConfigMsgBuffer,
                                        VehicleConfigMsgPayload vehicleConfigMsgBuffer,
                                    ...
                                    VehicleConfigMsgPayload vehicleConfigMsgBuffer)
{
    ...
    double dcm_RN[3][3];
    elem2rv(this->mu, &oe_d_dvn, rVec_d_dvn, vVec_d_dvn);
