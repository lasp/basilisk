...
void LocationPointing::UpdateState(uint64_t callTime)
{
    ...
    scTransInMsgBuffer = this->scTransInMsg();
    if (this->locationInMsg.isLinked()) {
        ...
        scTargetInMsgBuffer = this->scTargetInMsg();
    ...
    if (this->useBoresightRateDamping) {
    ...
    v3Copy(sigma_BR, this->sigma_BR_old);
