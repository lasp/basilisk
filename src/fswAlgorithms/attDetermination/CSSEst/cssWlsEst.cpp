...
{
    assert((this->cssConfigInMsgPayload.nCSS) < MAX_N_CSS_MEAS && "cssWIsEst.cssDataInMsg.nCSS must not be greater "
                                                                  "than MAX_N_CSS_MEAS value.");

    this->priorSignalAvailable = 0;
    v3SetZero(this->dOld);

    this->cssWlsFiltResOutMsgPayload.numObs = 0;
    this->cssWlsFiltResOutMsgPayload.timeTag = 0.0;
    v3SetZero(this->cssWlsFiltResOutMsgPayload.state);
    vSetZero(this->cssWlsFiltResOutMsgPayload.postFitRes, MAX_N_CSS_MEAS);
...
void CssWlsEst::UpdateState(uint64_t callTime)
{
    ...
    for(uint32_t i=0; i<this->cssConfigInMsgPayload.nCSS; i = i+1)
    {
        if(this->cssDataInMsgPayload.CosValue[i] > this->sensorUseThresh)
        {
            v3Scale(this->cssConfigInMsgPayload.cssVals[i].CBias,
                    this->cssConfigInMsgPayload.cssVals[i].nHat_B,
                    &H[this->numActiveCss*3]);
            y[this->numActiveCss] = cssDataInMsgPayload.CosValue[i];
            this->numActiveCss = this->numActiveCss + 1;
        }
    }

    /*! Estimation Steps*/
    this->cssWlsFiltResOutMsgPayload = {};
    if(this->numActiveCss == 0) /*! - If there is no sun, just quit*/
    {
        ...
        v3SetZero(this->navStateOutMsgPayload.vehSunPntBdy);       /* zero the sun heading to indicate now CSS info is available */
        v3SetZero(this->navStateOutMsgPayload.omega_BN_B);         /* zero the rate measure */
        this->priorSignalAvailable = 0;                       /* reset the prior heading estimate flag */
        computeWlsResiduals(cssDataInMsgPayload.CosValue,
                            &this->cssConfigInMsgPayload,
                            this->navStateOutMsgPayload.vehSunPntBdy,
                            this->cssWlsFiltResOutMsgPayload.postFitRes);
    } else {
        ...
        status = computeWlsmn((int) this->numActiveCss,
                              H,
                              W,
                              y,
                              this->navStateOutMsgPayload.vehSunPntBdy);
        computeWlsResiduals(this->cssDataInMsgPayload.CosValue,
                            &this->cssConfigInMsgPayload,
                            this->navStateOutMsgPayload.vehSunPntBdy,
                            this->cssWlsFiltResOutMsgPayload.postFitRes);

        v3Normalize(this->navStateOutMsgPayload.vehSunPntBdy, this->navStateOutMsgPayload.vehSunPntBdy);
        ...
        if (this->priorSignalAvailable && dt > 0.0) {
            ...
            v3Normalize(this->navStateOutMsgPayload.vehSunPntBdy, dHatNew);
            v3Normalize(this->dOld, dHatOld);
            v3Cross(dHatNew, dHatOld, this->navStateOutMsgPayload.omega_BN_B);
            v3Normalize(this->navStateOutMsgPayload.omega_BN_B,this->navStateOutMsgPayload.omega_BN_B);
            /* compute principal rotation angle between sun heading measurements */
            double dOldDotNew = v3Dot(dHatNew,dHatOld);
            if (dOldDotNew > 1.0) dOldDotNew = 1.0;
            if (dOldDotNew < -1.0) dOldDotNew = -1.0;
            v3Scale(safeAcos(dOldDotNew)/dt,
                    this->navStateOutMsgPayload.omega_BN_B,
                    this->navStateOutMsgPayload.omega_BN_B);
        } else {
            this->priorSignalAvailable = 1;
        }
        /*! -# Store the sun heading estimate */
        v3Copy(this->navStateOutMsgPayload.vehSunPntBdy, this->dOld);
    }

    /*! Residual Computation */
    this->cssWlsFiltResOutMsgPayload.numObs = (int) this->numActiveCss;
    this->cssWlsFiltResOutMsgPayload.timeTag = (double) (callTime*NANO2SEC);
    v3Copy(this->navStateOutMsgPayload.vehSunPntBdy, this->cssWlsFiltResOutMsgPayload.state);
    ...
    if(status > 0) /*! - If the status from the WLS computation is erroneous, populate the output messages with zeros*/
    {
        ...
        v3SetZero(this->navStateOutMsgPayload.vehSunPntBdy);       /* zero the sun heading to indicate anomaly  */
        v3SetZero(this->navStateOutMsgPayload.omega_BN_B);         /* zero the rate measure */
        this->priorSignalAvailable = 0;                       /* reset the prior heading estimate flag */
    }
...
void computeWlsResiduals(const double *cssMeas, CSSConfigMsgPayload *cssConfig, double *wlsEst, double *cssResiduals)
{
    ...
    for(uint32_t i=0; i<cssConfig->nCSS; i++)
    {
        ...
        double cssDotProd = v3Dot(wlsEst, cssConfig->cssVals[i].nHat_B);
