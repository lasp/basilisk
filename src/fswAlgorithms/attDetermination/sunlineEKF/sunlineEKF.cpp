 ...
 */
void SunlineEKF::reset(uint64_t callTime) {
    ...
    if (!this->cssConfigInMsg.isLinked()) {
        this->bskLogger.bskLog(BSK_ERROR, "Error: sunlineEKF.cssConfigInMsg wasn't connected.");
    }
    if (!this->cssDataInMsg.isLinked()) {
        this->bskLogger.bskLog(BSK_ERROR, "Error: sunlineEKF.cssDataInMsg wasn't connected.");
    }
    ...
    vSetZero(this->obs, (size_t) this->numObs);
    vSetZero(this->yMeas, (size_t) this->numObs);
    ...
    mSetZero(this->measMat, (size_t) this->numObs, this->numStates);
    mSetZero(this->kalmanGain, this->numStates, (size_t) this->numObs);

    mSetZero(this->measNoise, (size_t) this->numObs, (size_t) this->numObs);
    mSetIdentity(this->procNoise, this->numStates / 2, this->numStates / 2);
    mScale(this->qProcVal, this->procNoise, this->numStates / 2, this->numStates / 2, this->procNoise);
...
/*! This method takes the parsed CSS sensor data and outputs an estimate of the
 ...
 @param callTime The clock time at which the function was called (nanoseconds)
 */
void SunlineEKF::updateState(uint64_t callTime) {
    double newTimeTag;
    ...
    if (newTimeTag > this->timeTag) {
        this->sunlineTimeUpdate(newTimeTag);
        vCopy(this->xBar, SKF_N_STATES, this->x);
        mCopy(this->covarBar, SKF_N_STATES, SKF_N_STATES, this->covar);
    }
...
*/
void SunlineEKF::sunlineTimeUpdate(double updateTime) {
    ...
    sunlineDynMatrix(this->state, this->dt, this->dynMat);
    sunlineStateSTMProp(this->dynMat, this->dt, this->state, this->stateTransition);

    /* xbar = Phi*x */
    mMultV(this->stateTransition, SKF_N_STATES, SKF_N_STATES, this->x, this->xBar);

    /*! - Update the covariance */
    /*Pbar = Phi*P*Phi^T + Gamma*Q*Gamma^T*/
    mTranspose(this->stateTransition, SKF_N_STATES, SKF_N_STATES, stmT);
    mMultM(this->covar, SKF_N_STATES, SKF_N_STATES, stmT, SKF_N_STATES, SKF_N_STATES, covPhiT);
    mMultM(this->stateTransition, SKF_N_STATES, SKF_N_STATES, covPhiT, SKF_N_STATES, SKF_N_STATES, this->covarBar);

    /*Compute Gamma and add gammaQGamma^T to Pbar. This is the process noise addition*/
    //    double
    //    Gamma[6][3]={{this->dt*this->dt/2,0,0},{0,this->dt*this->dt/2,0},{0,0,this->dt*this->dt/2},{this->dt,0,0},{0,this->dt,0},{0,0,this->dt}};
    mSetIdentity(Id, SKF_N_STATES_HALF, SKF_N_STATES_HALF);
    mScale(this->dt, Id, SKF_N_STATES_HALF, SKF_N_STATES_HALF, Id);
    mSetSubMatrix(Id, 3, 3, Gamma, 6, 3, 3, 0);
    mScale(this->dt / 2, Id, SKF_N_STATES_HALF, SKF_N_STATES_HALF, Id);
    mSetSubMatrix(Id, 3, 3, Gamma, 6, 3, 0, 0);

    mMultMt(this->procNoise, SKF_N_STATES_HALF, SKF_N_STATES_HALF, Gamma, SKF_N_STATES, SKF_N_STATES_HALF, qGammaT);
    mMultM(Gamma, SKF_N_STATES, SKF_N_STATES_HALF, qGammaT, SKF_N_STATES_HALF, SKF_N_STATES, gammaQGammaT);
    mAdd(this->covarBar, SKF_N_STATES, SKF_N_STATES, gammaQGammaT, this->covarBar);
...
/*! This method performs the measurement update for the sunline kalman filter.
 ...
 @param updateTime The time that we need to fix the filter to (seconds)
 */
void SunlineEKF::sunlineMeasUpdate(double updateTime) {
    /*! - Compute the valid observations and the measurement model for all observations*/
    sunlineHMatrixYMeas(
        this->state,
        (int) this->numCSSTotal,
    ...
    sunlineKalmanGain(this->covarBar, this->measMat, this->qObsVal, this->numObs, this->kalmanGain);

    /* Logic to switch from EKF to CKF. If the covariance is too large, switching
     * references through an EKF could lead to filter divergence in extreme cases.
     * In order to remedy this, past a certain infinite norm of the covariance,
     * we update with a CKF in order to bring down the covariance. */

    if (vMaxAbs(this->covar, SKF_N_STATES * SKF_N_STATES) > this->eKFSwitch) {
        ...
        /*! - Compute the update with a EKF, notice the reference state
         * is added as an argument because it is changed by the filter update */
        sunlineEKFUpdate(
            this->kalmanGain,
            this->covarBar,
            this->qObsVal,
...
void sunlineCKFUpdate(
    ...
    double measMatx[MAX_NUM_CSS_SENSORS], innov[MAX_NUM_CSS_SENSORS], kInnov[SKF_N_STATES];
    ...
    double eyeKalHCovarBar[SKF_N_STATES * SKF_N_STATES], kalR[SKF_N_STATES * MAX_NUM_CSS_SENSORS];
    double kalT[MAX_NUM_CSS_SENSORS * SKF_N_STATES], kalRKalT[SKF_N_STATES * SKF_N_STATES];
    ...
    mSetZero(kalT, MAX_NUM_CSS_SENSORS, SKF_N_STATES);
    mSetZero(kalR, SKF_N_STATES, MAX_NUM_CSS_SENSORS);
...
void sunlineKalmanGain(
    ...
    double hObsT[SKF_N_STATES * MAX_NUM_CSS_SENSORS];
    double covHT[SKF_N_STATES * MAX_NUM_CSS_SENSORS];
    double hCovar[MAX_NUM_CSS_SENSORS * SKF_N_STATES], hCovarHT[MAX_NUM_CSS_SENSORS * MAX_NUM_CSS_SENSORS];
    double rMat[MAX_NUM_CSS_SENSORS * MAX_NUM_CSS_SENSORS];

    /* Setting all local variables to zero */
    mSetZero(hObsT, SKF_N_STATES, MAX_NUM_CSS_SENSORS);
    mSetZero(covHT, SKF_N_STATES, MAX_NUM_CSS_SENSORS);
    mSetZero(hCovar, MAX_NUM_CSS_SENSORS, SKF_N_STATES);
