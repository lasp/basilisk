...
void ThrustCMEstimation::Reset(uint64_t CurrentSimNanos)
{
    ...
    this->sBar = ThrustCMEstimation::choleskyDecomposition(this->covar);
    this->cholProcessNoise = ThrustCMEstimation::choleskyDecomposition(this->processNoise * 1E-6);
}
 ...
 */
void ThrustCMEstimation::timeUpdate(double updateTime)
{
    ...
    this->sBar = ThrustCMEstimation::qrDecompositionJustR(A);
    ...
    this->sBar = ThrustCMEstimation::choleskyUpDownDate(this->sBar, xError, this->wC(0));
 ...
 * It updates class variables relating to measurement data including validity and time tags.
 @return void
 */
void ThrustCMEstimation::writeOutputMessages(uint64_t CurrentSimNanos) {
    /*
 ...
 * It updates class variables relating to measurement data including validity and time tags.
 @return void
 */
void ThrustCMEstimation::readFilterMeasurements() {
    /*
    ...
    if (this->opNavHeadingBuffer.valid){
        ...
        this->measurementNoise = this->measNoiseScaling * cArray2EigenMatrixXd(this->opNavHeadingBuffer.covar_N,
                                                                               (int) this->obs.size(),
                                                                               (int) this->obs.size());
        this->cholMeasurementNoise = ThrustCMEstimation::choleskyDecomposition(this->measurementNoise);
    } */
}
...
/*! Compute the measurement model.  Given that the data is coming from
 ...
 @return void
 */
void ThrustCMEstimation::measurementModel()
{
...
@return void
 */
void ThrustCMEstimation::computePostFitResiudals()
{
...
/*! Perform the measurement update for the kalman filter.
 ...
 @return void
 */
void ThrustCMEstimation::measurementUpdate()
{
    ...
    sy = ThrustCMEstimation::qrDecompositionJustR(A);

    /*! - Cholesky update block for vectors.*/
    Eigen::VectorXd yError = this->yMeas.col(0) - yBar;
    sy = ThrustCMEstimation::choleskyUpDownDate(sy, yError, this->wC(0));
    ...
    STkMatT = ThrustCMEstimation::forwardSubstitution(sy, pXY.transpose());
    kMat = ThrustCMEstimation::backSubstitution(sy.transpose(), STkMatT).transpose();
    ...
    {
        this->sBar = ThrustCMEstimation::choleskyUpDownDate(this->sBar, Umat.col(i), -1);
    }
...
*/
Eigen::VectorXd ThrustCMEstimation::propagate(std::array<double, 2> interval, const Eigen::VectorXd& X0, double dt) const
{
...
/*! Runge-Kutta 4 (RK4) function for the state propagation
    ...
    @return Eigen::VectorXd
*/
Eigen::VectorXd ThrustCMEstimation::rk4(const std::function<Eigen::VectorXd(double, Eigen::VectorXd)>& ODEfunction,
 ...
 */
Eigen::MatrixXd ThrustCMEstimation::qrDecompositionJustR(const Eigen::MatrixXd input) const
{
 ...
 */
Eigen::MatrixXd ThrustCMEstimation::choleskyUpDownDate(const Eigen::MatrixXd input,
                                               ...
                                               const double coefficient) const
{
    ...
    Eigen::MatrixXd A;
    A = ThrustCMEstimation::choleskyDecomposition(P);
 ...
 */
Eigen::MatrixXd ThrustCMEstimation::choleskyDecomposition(const Eigen::MatrixXd input) const
{
 ...
 */
Eigen::MatrixXd ThrustCMEstimation::backSubstitution(const Eigen::MatrixXd U, const Eigen::MatrixXd b) const
{
...
/*! Perform a generic forward substitution to solve x in Lx = b
 ...
 @param Eigen::MatrixXd b, the right hand side of the Ux = b
 */
Eigen::MatrixXd ThrustCMEstimation::forwardSubstitution(const Eigen::MatrixXd L, const Eigen::MatrixXd b) const
{
