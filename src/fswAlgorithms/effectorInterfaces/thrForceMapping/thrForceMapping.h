...
typedef struct {
    ...
    float   controlAxes_B[3*3];                    //!< []      array of the control unit axes
    float   rThruster_B[MAX_EFF_CNT][3];           //!< [m]     local copy of the thruster locations
    float   gtThruster_B[MAX_EFF_CNT][3];          //!< []      local copy of the thruster force unit direction vectors
    int32_t  thrForceSign;                          //!< []      Flag indicating if pos (+1) or negative (-1) thruster solutions are found
    float angErrThresh;                            //!< [r]     Angular error at which thruster forces are scaled to not be super-saturated
    float   epsilon;                               //!<         variable specifying what is considered a small number
    ...
    float outTorqAngErr;                           //!< [r]     Angular error of effector torque
    float thrForcMag[MAX_EFF_CNT];                 //!<         vector of thruster force magnitudes
...
extern "C" {
    ...
    void findMinimumNormForce(thrForceMappingConfig *configData,
                              float D[3][MAX_EFF_CNT], float Lr_B[3], uint32_t numForces, float F[MAX_EFF_CNT]);
    float computeTorqueAngErr(float D[3][MAX_EFF_CNT], float BLr[3], uint32_t numForces, float epsilon,
