...
def test_stepperMotorProfilerTestFunction(show_plots, initialMotorAngle, stepsCommanded, stepAngle, stepTime, accuracy):
    ...
    **Validation Test Description**
    ...
    This unit test ensures that the stepper motor profiler module correctly actuates the stepper motor by the incoming
    integer number of commanded steps contained in the ``motorStepCount`` input message. The initial motor angle and
    number of commanded steps are varied so that the module is shown to work for both positive and negative steps.
    The motor states are profiled for each step using a bang-bang constant positive, constant negative acceleration
    profile. The motor acceleration is determined from the given constant step angle and constant step time.
    ...
    **Description of Variables Being Tested**
    ...
    commanded from the module `motorStepCount`` input message.
...
def stepperMotorProfilerTestFunction(show_plots, initialMotorAngle, stepsCommanded, stepAngle, stepTime, accuracy):
    ...
    MotorStepCountMessageData.stepsCommanded = stepsCommanded
    MotorStepCountMessage = messaging.MotorStepCountMsg().write(MotorStepCountMessageData)
    StepperMotorProfiler.motorStepCountInMsg.subscribeTo(MotorStepCountMessage)
